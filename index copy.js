...
async function processEmails(company, auth, currentCursorTime) {
    ...
    if (messages.length === 0) {
        console.log(`[${company}] No new emails found.`);
    ...
    for (const message of messages) {
        try {
            ...
            if (!encodedBody) {
                console.log(`[${company}] [${message.id}] Body missing. Marking processed (skip).`);
            ...
            if (finalData.amount && finalData.orderId && finalData.accountOf) {
                ...
                console.log(`[${company}] [${message.id}] SKIPPED: Missing required fields (Amount/ID/AccountOf).`);
                console.log(`[${company}] Debug Data: Amount=${finalData.amount}, OrderId=${finalData.orderId}, Account=${finalData.accountOf}`);
...
async function startPollingForCompany(company) {
  ...
  while (true) {
    try {
      ...
      const newTime = await processEmails(
        company,
        auth,
        localCursorTime
      );
      ...
      console.error(`[${company}] ❌ Loop error: ${err.message}`);
    }
...
(async () => {
  ...
  for (const company of COMPANIES) {
    startPollingForCompany(company);
    await sleep(15000); // slight stagger to avoid quota spikes
  }
})();
